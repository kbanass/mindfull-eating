...
export function Nav({ currentTab, tabChangeHandler }: NavProps) {
  return (
    <nav className={styles.nav}>
      <motion.button
        whileTap={{ scale: 0.8 }}
        className={`${styles.button} ${currentTab === "today" && styles.active}`}
        onPointerDown={() => tabChangeHandler("today")}
      >
        <svg
      ...
      <motion.button
        whileTap={{ scale: 0.8 }}
        className={`${styles.button} ${currentTab === "month" && styles.active}`}
        onPointerDown={() => tabChangeHandler("month")}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width={24}
      ...
      <motion.button
        whileTap={{ scale: 0.8 }}
        className={`${styles.button} ${currentTab === "profile" && styles.active}`}
        onPointerDown={() => tabChangeHandler("profile")}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width={24}
